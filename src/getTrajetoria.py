...
def resample_polyline_arclength(xy: np.ndarray, n_out: int) -> np.ndarray:
    """Reamostra uma polilinha 2D em `n_out` pontos igualmente espaçados em comprimento de arco."""
    if len(xy) < 2:
...
def comprimento_arco(xy: np.ndarray) -> np.ndarray:
    """Comprimento de arco acumulado de uma polilinha 2D."""
    if len(xy) < 2:
...
def interpolar_por_arco(xy: np.ndarray, s_query: np.ndarray) -> np.ndarray:
    """Interpola pontos da polilinha para distâncias acumuladas s_query."""
...
def distancia_acumulada_por_velocidade(t: np.ndarray, vel: np.ndarray) -> np.ndarray:
    """Integra |vel| para obter progresso escalar acumulado do veículo."""
    s = np.zeros(len(t))
...
def estimar_periodo_volta_segundos(t: np.ndarray, vel: np.ndarray) -> tuple[float | None, str]:
    """
    Estima o período de uma volta pela autocorrelação da velocidade (magnitude).
    Retorna (T_em_segundos, motivo) com motivo explicando falhas.
    """
    if len(vel) < 100:
...
def rastrear_no_mapa_primeira_volta(
    ...
    """
    Regra de negócio do mapa:
    - primeira volta integral define a geometria fixa do mapa;
    - voltas seguintes não geram uma pista nova;
    - a posição publicada é o progresso do carro projetado nesse mapa fixo.
    """
